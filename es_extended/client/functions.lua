...
ESX.Game.GetVehicleProperties = function(vehicle)
	...
	return {
		...
		color1            = color1,
		color2            = color2,
                headlightsColor   = GetVehicleHeadlightsColour(vehicle),
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	...
	if props.color2 ~= nil then
        ...
        if props.headlightsColor ~= nil then
	        SetVehicleHeadlightsColour(vehicle, props.headlightsColor)
        end
...
CheckWeapons = function(item)
	if 
	(tostring(item) == tostring('pistol')
	or tostring(item) == tostring('pistol_mk2')
	or tostring(item) == tostring('snspistol_mk2')
	or tostring(item) == tostring('snspistol')
	or tostring(item) == tostring('vintagepistol')
	or tostring(item) == tostring('nightstick')
	or tostring(item) == tostring('stungun')
	or tostring(item) == tostring('flashlight')
	or tostring(item) == tostring('knife')
	or tostring(item) == tostring('switchblade'))
	then
		return true
	else
		return false
...
--			table.insert(elements, {label = _U('giveammo'), action = 'giveammo', type = data.current.type, value = data.current.value})
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item',
		{
            ...
            local playerPed = PlayerPedId()
            local number = data1.current.number
...
openBindsMenu = function(datavalue)
	...
	ESX.TriggerServerCallback('bind:getBinds', function(a,b,c,d,e)
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'bagol',
		{
			...
			TriggerServerEvent('bind:RegisterItemToSlot', data.current.what, item)
			openBindsMenu(item)
		end,
	function(data, menu)
		menu.close()
