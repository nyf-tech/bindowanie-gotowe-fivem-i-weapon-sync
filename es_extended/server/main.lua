...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				...
				sendToDiscord('es_extended','[Dawanie] ' .. sourceXPlayer.name .. ' Dał,  ' .. targetXPlayer.name .. ' ITEM: ' .. itemName .. ' ILOŚĆ: ' ..itemCount, 16711680)
			end
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			sendToDiscord('es_extended','[Dawanie] ' .. sourceXPlayer.name .. ' Dał hajs,  ' .. targetXPlayer.name .. ' ILOŚĆ: ' ..itemCount, 16711680)
		else
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sendToDiscord('es_extended','[Dawanie] ' .. sourceXPlayer.name .. ' Dał brudny hajs,  ' .. targetXPlayer.name .. ' ILOŚĆ: ' ..itemCount, 16711680)
		else
			TriggerClientEvent('esx:showNotification', _source, _U('imp_invalid_amount'))
		end
	...
	elseif type == 'item_weapon' then
		...
		sendToDiscord('es_extended','[Dawanie] ' .. sourceXPlayer.name .. ' Dał broń, ' .. targetXPlayer.name .. ' BROŃ: ' .. weaponLabel .. ' ILOŚĆ AMMO: ' ..itemCount, 16711680)
	end
	...
	if type == 'item_standard' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				sendToDiscord('es_extended','[Wyrzucenie] ' .. GetPlayerName(source) .. ' Wyrzucił‚ ITEM: ' .. xItem.label .. ' ILOŚĆ: ' .. itemCount, 2096865)
	...
	elseif type == 'item_money' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > playerCash or playerCash < 1) then
				...
				sendToDiscord('es_extended','[Wyrzucenie] ' .. GetPlayerName(source) .. ' Wyrzucił‚ gotówkę, ILOŚĆ: ' .. itemCount, 2096865)
			end
		end
	...
	elseif type == 'item_account' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				sendToDiscord('es_extended','[Wyrzucenie] ' .. GetPlayerName(source) .. ' Wyrzucił‚ brudną gotówkę, ILOŚĆ: ' .. itemCount, 2096865)
			end
		end
	...
	elseif type == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if itemCount > 0 then
				...
				sendToDiscord('es_extended','[Wyrzucenie] ' .. GetPlayerName(source) .. ' Wyrzucił‚ BROŃ: ' .. itemName .. ' ILOŚĆ: ' .. itemCount, 2096865)
...
AddEventHandler('esx:useItem', function(itemName)
	...
	if count > 0 then
		...
		sendToDiscord('es_extended','[Używa] ' .. GetPlayerName(source) .. ' Używa‚ ITEM: ' .. itemName, 2096865)
	else
...
AddEventHandler('esx:onPickup', function(id)
	...
	if pickup.type == 'item_standard' then
		...
		if total > 0 then
			...
			sendToDiscord('es_extended','[Podnoszenie] ' .. GetPlayerName(source) .. ' Podnosi‚ ITEM: ' .. pickup.name .. ' ILOŚĆ: ' .. total, 2096865)
		end
	...
	elseif pickup.type == 'item_money' then
		...
		sendToDiscord('es_extended','[Podnoszenie] ' .. GetPlayerName(source) .. ' Podnosi‚ gotówkę ILOŚĆ: ' .. pickup.count, 2096865)
	elseif pickup.type == 'item_account' then
		TriggerClientEvent('esx:removePickup', -1, id)
		xPlayer.addAccountMoney(pickup.name, pickup.count)
		sendToDiscord('es_extended','[Podnoszenie] ' .. GetPlayerName(source) .. ' Podnosi‚ brudną gotówkę ILOŚĆ: ' .. pickup.count, 2096865)
	end
end)
...
function sendToDiscord (name,message,color)
  local DiscordWebHook = 'https://discord.com/api/webhooks/834772394614325258/6xAwheAZAIYjFEvpGUIlyaW9yo1J4erGcso9p3Of8GhLVFCjcQceceiSvm7N2tusmtna'
